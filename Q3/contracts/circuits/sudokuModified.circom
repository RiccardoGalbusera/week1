...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            puzzleRange[i][j].out === 1; //[assignment] change assert() to use your created RangeProof instead

            solutionRange[i][j] = RangeProof(32);
            solutionRange[i][j].in <== solution[i][j];
            solutionRange[i][j].range[0] <== 0;
            solutionRange[i][j].range[1] <== 9;
            solutionRange[i][j].out === 1; //[assignment] change assert() to use your created RangeProof instead
